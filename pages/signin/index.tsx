          ...
          if (path.status == 200) {
            ...
            for (var idx in pathList) {
              if (pathList[idx].pathName === "store") {
                ...
                dispatch(
                  setDefaultPrivatePath(pathList[idx].privateDefaultPath)
                );
                dispatch(setNowPathToPublicPath());
              } else if (pathList[idx].pathName === "trash") {
                dispatch(
                  setDefaultPublicTrashPath(pathList[idx].publicDefaultPath)
                );
                dispatch(
                  setDefaultPrivateTrashPath(pathList[idx].privateDefaultPath)
                );
              } else if (pathList[idx].pathName === "thumbnail") {
                dispatch(
                  setDefaultThumbnailPath(pathList[idx].publicDefaultPath)
                );
              }
            }
            router.push("/main");
        ...
        <div>
          <Container>
            <div className="d-flex justify-content-center ">
              <Image alt="Logo" src={Logo} width="135" height="135" />
            </div>
